...
//  *Data = SPI_Mems_Read_Reg( Reg );
	Spi_LSM303D_MultiRead(Reg,Data,1);
...
u8_t WriteReg(u8_t deviceAddress, u8_t WriteAddr, u8_t Data) {
  ...
  Spi_LSM303_SingleWrite(WriteAddr, Data);
...
* Return         : Status [MEMS_ERROR, MEMS_SUCCESS]
*******************************************************************************/
status_t GetAccAxesRaw(Vector3f_t* buff) {
//  u8_t valueL;
//  u8_t valueH;
	u8_t Data[6];
	
  Spi_LSM303D_MultiRead(OUT_X_L_A,Data,6);
	buff->x = ( (Data[1] << 8) | Data[0] )/16.0f;
	buff->y = ( (Data[3] << 8) | Data[2] )/16.0f;
	buff->z = ( (Data[5] << 8) | Data[4] )/16.0f;
...
void lsm303d_init(void){
	uint8_t response;
 response = SetODR(ODR_25Hz);SoftDelayMs(50);
// if(response==1){
//        sprintf((char*)buffer,"\n\rSET_ODR_OK    \n\r\0");
...
//  }
 ...
 response = SetODR_M(ODR_220Hz_M);SoftDelayMs(50);
// if(response==1){
//        sprintf((char*)buffer,"\n\rSET_ODR_M_OK  \n\r\0");
//        USB_SIL_Write(EP1_IN, buffer, 19);
//        SetEPTxValid(ENDP1);
//  }
 //set PowerMode 
 response = SetMode(NORMAL);SoftDelayMs(50);
// if(response==1){
...
//  }
 ...
 response = SetModeMag(CONTINUOUS_MODE);SoftDelayMs(50);
// if(response==1){
...
//  }
 //set Fullscale
 response = SetFullScale(FULLSCALE_2);SoftDelayMs(50);
// if(response==1){
...
//  }
 ...
 response = SetGainMag(GAIN_450_M);SoftDelayMs(50);
// if(response==1){
...
//  }
 //set axis Enable
 response = SetAxis(X_ENABLE | Y_ENABLE | Z_ENABLE);SoftDelayMs(50);
// if(response==1){
